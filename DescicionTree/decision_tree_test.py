...
class DecisionTree:
    def __init__(self, min_sample_split=2, max_depth=100, n_feat=None):
    ...
    def fit(self, X, y):
        # grow tree
        self.n_feat = X.shape[1] if not self.n_feat else min(
    ...
    def __grow_tree(self, X, y, depth=0):
        ...
        # greedy search
        best_feature, best_thresh = self.__best_criteria(X, y, feat_idxs)
        left_idxs, right_idxs = self.__split(X[:, best_feature], best_thresh)
    ...
    def __information_gain(self, y, X_column, split_thresh):
        # parent E
